...
export default {
  ...
  setup() {
    ...
    // vee-validate校验基本步骤
    // 1.导入Form,Field组件 将form和input进行替换  Field 添加name属性，作用是指定使用schema中哪个校验规则
    // 2.Field需要进行数据绑定,字段名称与后台借口保持一致 name='account' v-model="form.accout"
    // 3.定义Field的name属性指定的校验规则函数，Form的validation-schema接收定义好的校验规则 是个对象
    // 4.自定义组件校验必须支持v-model 然后Field使用as指定为组件名称
    const mySchema = {
      ...
      if (valid) {
        try {
          ...
          if (isMsgLogin.value) {
            ...
            // 2.1 准备一个API做手机号登陆
            // 2.2 准备API函数
            // 2.3 成功：存储用户信息 + 跳转至来源页 或 首页 + 消息提示
            // 2.4 失败：消息提示
